{ config, lib, pkgs, modulesPath, nixos-hardware, ... }: {
  ...
  options = {
    citadel.machines.common = {
      enable = lib.mkEnableOption "Citadel: Common Machine Configuration";
      partition = {
        efi = lib.mkOption {
          type = lib.types.str;
          default = "/dev/sda1";
  ...
  config = lib.mkIf config.citadel.machines.common.enable {
    boot.initrd.availableKernelModules =
      ...
      boot.initrd.luks.devices = {
        boot = {
          device = config.citadel.machines.common.partition.boot;
          allowDiscards = true;
        };

        root = {
          device = config.citadel.machines.common.partition.root;
          preLVM = true;
          allowDiscards = true;
        };
      };
      ...
      fileSystems."/efi" = {
        device = config.citadel.machines.common.partition.efi;
        fsType = "vfat";
      };
